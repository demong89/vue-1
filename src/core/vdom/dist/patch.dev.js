...
function createPatchFunction(backend) {
  ...
  var modules = backend.modules,
      nodeOps = backend.nodeOps; // 把模块中的钩子函数全部设置到cbs中 ， 将来统一触发
  ...
  function patchVnode(oldVnode, vnode, insertedVnodeQueue, ownerArray, index, removeOnly) {
    ...
    var ch = vnode.children; // 触发update钩子函数
    ...
    if ((0, _index.isUndef)(vnode.text)) {
      if ((0, _index.isDef)(oldCh) && (0, _index.isDef)(ch)) {
        ...
        } // 如果新节点有子节点 并且旧节点有text 清空旧节点对应的真实DOM的文本内容


        if ((0, _index.isDef)(oldVnode.text)) nodeOps.setTextContent(elm, ''); // 把新节点的子节点转换成正式DOM 添加到elm

        addVnodes(elm, null, ch, 0, ch.length - 1, insertedVnodeQueue);
      } else if ((0, _index.isDef)(oldCh)) {
        // 如果旧节点有子节点 新节点没有 移除所有旧节点对应的真实DOM
        removeVnodes(oldCh, 0, oldCh.length - 1);
      } else if ((0, _index.isDef)(oldVnode.text)) {
        // 如果旧节点有text 新节点没有子节点和text
        nodeOps.setTextContent(elm, '');
      }
    } else if (oldVnode.text !== vnode.text) {
      // 如果新节点有text 并且和旧节点的text不同
      // 直接把新节点的text更新到DOM上
  ...
  return function patch(oldVnode, vnode, hydrating, removeOnly) {
    // 如果没有vnode但是有oldVnode ，执行销毁的钩子函数
    ...
    if ((0, _index.isUndef)(oldVnode)) {
      ...
      // 判断当前oldVnode是否是DOM元素 （首次渲染）
      var isRealElement = (0, _index.isDef)(oldVnode.nodeType);

      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        // patch existing root node
        // 如果不是真实DOM 并且两个VNode是 sameVNode 这个时候开始执行diff
